# Root AGENTS.md — AKSEP Submodule

These rules cover the entire AKSEP repository (this Git submodule). A nested `AGENTS.md` overrides the root instructions for its subtree; obey the closest file.
...
- All Git operations (**status, add, checkout/restore, stash, commit**) must be **pathspec-limited** to Task Scope Paths.
  Examples:
  - `git status --porcelain -- projects/AKSEP/ TASK_PLAN.md TASK_DOCS.md`
  - `git add -- docs/ TASK_PLAN.md TASK_DOCS.md`
  - `git stash push -m "TASK:<id> scope" -- projects/AKSEP/`
...
2) If a listed item was **renamed/moved/retired** but the new path is obvious (e.g., `projects/AKSEP/content/` becomes `projects/AKSEP/site-content/` or a folder is no longer a submodule), you may:
...
- **Title format**: `[AKSEP] <scope> (<k>/<n>)` - e.g., `[AKSEP] Content cleanup (1/2)`.
...
- projects/AKSEP/**, projects/schoolsystem_DB/**, docs/** (path from repo root, one folder path per bullet as required)
- TASK_PLAN.md
- TASK_DOCS.md
...
- **`projects/AKSEP/`** — Nuxt 3 application for the AKSEP website. Contains its own `AGENTS.md`; defer to it for Nuxt/content specifics.
- **`projects/AKSEP-ALT/`** — Legacy Firebase/Eleventy prototype. Only touch when explicitly asked; treat generated files with care.
- **`projects/schoolsystem_DB/`** — Documentation for a database exercise (Markdown only).
- **Root docs & assets**: `README.md` (repo bootstrap), `Values.txt` (color palette), `docs/index.html` (published overview).
- **Tooling**: root `package.json` with `npm run AKSEP` to install and start the Nuxt preview (`projects/AKSEP`). Parent repo manages `.gitmodules`.
- **Generated / do not commit**: `.nuxt/`, `.output/`, `node_modules/`, `projects/AKSEP/.vercel/`, and other build artefacts.

**Repo safety rails**
- Stay within this submodule’s sandbox; do not modify parent repo files.
- External registries allowed during setup: `registry.npmjs.org`.
...
- **Indentation**: 2 spaces for JS/TS/Vue in `projects/AKSEP/`; respect existing style in legacy folders (`projects/AKSEP-ALT`, etc.).
- **Naming**: Nuxt/Vue components in PascalCase; composables/composers in camelCase; files in kebab-case unless the framework dictates otherwise.
- **Language**: English for code, comments, and commit messages; German is acceptable inside content Markdown front-matter/variants.
- **TypeScript**: keep or enable `strict` mode where available; avoid `any`; annotate exports with JSDoc when types need explanation.
- **CSS/tokens**: reuse variables defined in `Values.txt` or Nuxt design tokens when introducing new colors.
- **Content edits**: prefer working through Markdown/i18n files (see nested `projects/AKSEP/AGENTS.md`).
...
for p in projects/AKSEP projects/AKSEP-ALT projects/schoolsystem_DB Values.txt docs/index.html; do
  [ -e "$p" ] || echo "AGENTS.md map mismatch: missing $p"
done
```

**Node/JS/TS** (run inside the target project folder, e.g., `projects/AKSEP/` when `package.json` is present):
```bash
...
Root convenience: `npm run AKSEP` (from repo root) installs dependencies and starts the Nuxt preview via `projects/AKSEP/`.

**Python** (not currently used in this submodule): skip unless a future folder adds the tooling.

**Eleventy sites** (legacy `projects/AKSEP-ALT/` or other Eleventy folders when explicitly touched):
```bash
$PM run build || :
$PM run serve || :
```
...
* **PR title**: `[AKSEP]: <short description>`; avoid special chars (expand umlauts: `ä->ae`, etc.).
...
- [ ] No changes under generated paths (`projects/AKSEP/.nuxt/**`, `projects/AKSEP/.output/**`, `projects/AKSEP/node_modules/**`, `node_modules/**`)
